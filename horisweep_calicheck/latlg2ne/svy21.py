...
class SVY21:
    ...
    def calcM(self, lat):
        latR = lat * np.pi / 180
        return self.a * ((self.A0 * latR) - (self.A2 * np.sin(2 * latR)) + (self.A4 * np.sin(4 * latR)) - (self.A6 * np.sin(6 * latR)))

    def calcRho(self, sin2Lat):
        num = self.a * (1 - self.e2)
        denom = np.power(1 - self.e2 * sin2Lat, 3. / 2.)
        return num / denom

    def calcV(self, sin2Lat):
        poly = 1 - self.e2 * sin2Lat
        return self.a / np.sqrt(poly)
    ...
    def computeLatLon(self, N, E):
        ...
        n3 = n2 * n
        n4 = n2 * n2
        G = self.a * (1 - n) * (1 - n2) * (1 + (9 * n2 / 4) + (225 * n4 / 64)) * (np.pi / 180)
        sigma = (Mprime * np.pi) / (180. * G)
        
        latPrimeT1 = ((3 * n / 2) - (27 * n3 / 32)) * np.sin(2 * sigma)
        latPrimeT2 = ((21 * n2 / 16) - (55 * n4 / 32)) * np.sin(4 * sigma)
        latPrimeT3 = (151 * n3 / 96) * np.sin(6 * sigma)
        latPrimeT4 = (1097 * n4 / 512) * np.sin(8 * sigma)
        ...
        lon = (self.oLon * np.pi / 180) + lonTerm1 - lonTerm2 + lonTerm3 - lonTerm4
