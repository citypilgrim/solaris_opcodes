...
def main(
    ...
    (un)comment accordingly the map plotting below
    ...
    ## plotting graphical map
    onemap_dir = '/home/tianli/SOLAR_EMA_project/codes/smmpl_codes/horisweep_plot/cadastral_trimmer/connexis_onemap.png'
    sg_img = pil.Image.open(onemap_dir)
    sglen, sgheight = sg_img.size[0], sg_img.size[1]
    scale = 1/185000            # values set here are specific to the image
    sglen, sgheight = scale * sglen, scale * sgheight
    olg, olt = 103.783470, 1.29718
    ax.imshow(sg_img, extent=(olg, olg+sglen, olt, olt+sgheight))
    ax.plot(103.7877882, 1.2997406, 'ro', markersize=1) # help to align map
    ax.plot(103.7880778, 1.2988504, 'ro', markersize=1)
    lowx, highx, lowy, highy = olg, olg+sglen, olt, olt+sgheight
...
if __name__ == '__main__':
    ...
    # nrbd_l.append(nrb_calc(
    #     'smmpl_E2', smmpl_reader,
    #     date=pd.Timestamp('20200707'),
    #     starttime=LOCTIMEFN('202007070343', UTCINFO),
    #     endtime=LOCTIMEFN('202007070630', UTCINFO),
    #     genboo=True,
    # ))
    nrbd_l.append(nrb_calc(
        'smmpl_E2', smmpl_reader,
